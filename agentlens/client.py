"""SDK client: routes spans either directly into a local SQLite store (no
server needed) or to a running AgentLens server over HTTP, batched on a
background thread with flush-on-exit."""
...
class LocalClient:
    """Writes spans straight to ./.agentlens/agentlens.db (or a given path)."""
...
class HttpClient:
    """Buffers spans and POSTs them to the AgentLens server in batches."""
    ...
    def __init__(self, tracking_uri: str, flush_interval: float = 1.0, batch_size: int = 100):
        ...
        self._thread = threading.Thread(target=self._worker, daemon=True, name="agentlens-flush")
        self._thread.start()
...
def init(project: str = "default", tracking_uri: Optional[str] = None, db_path: Optional[str] = None):
    """Initialize AgentLens. With no ``tracking_uri`` spans are written directly
    to a local SQLite file, MLflow-style. With no ``db_path`` the store defaults
    to a hidden ``./.agentlens/agentlens.db``."""
