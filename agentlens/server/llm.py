"""Pluggable LLM generation backends for the optional "replay" feature.

AgentLens is a diagnostics tool, but it can also *re-run* a prompt captured by a
trace — useful for debugging an agent decision or comparing models. It is
provider-agnostic by design: rather than depend on a different SDK per vendor,
every backend goes through ``httpx`` (already a core dependency). The broad
OpenAI-compatible ``/chat/completions`` shape covers OpenAI, Gemini, Groq,
OpenRouter, Together, Mistral, DeepSeek, Fireworks and Perplexity; Anthropic
uses its native ``/v1/messages`` API and Ollama its local ``/api/chat``.
Providers are detected at runtime from environment variables so the tool stays
zero-config and offline by default.
"""
...
PROVIDERS: Dict[str, Dict[str, Any]] = {
    ...
    "openai": {"label": "OpenAI", "kind": "openai", "env": ["OPENAI_API_KEY"],
               "base": "https://api.openai.com/v1", "default_model": "gpt-4o-mini"},
    "gemini": {"label": "Google Gemini", "kind": "openai", "env": ["GEMINI_API_KEY", "GOOGLE_API_KEY"],
               "base": "https://generativelanguage.googleapis.com/v1beta/openai",
               "default_model": "gemini-2.0-flash"},
    "groq": {"label": "Groq", "kind": "openai", "env": ["GROQ_API_KEY"],
             "base": "https://api.groq.com/openai/v1", "default_model": "llama-3.3-70b-versatile"},
    "openrouter": {"label": "OpenRouter", "kind": "openai", "env": ["OPENROUTER_API_KEY"],
                   "base": "https://openrouter.ai/api/v1", "default_model": "openai/gpt-4o-mini"},
    "together": {"label": "Together AI", "kind": "openai", "env": ["TOGETHER_API_KEY"],
                 "base": "https://api.together.xyz/v1",
                 "default_model": "meta-llama/Llama-3.3-70B-Instruct-Turbo"},
    "mistral": {"label": "Mistral", "kind": "openai", "env": ["MISTRAL_API_KEY"],
                "base": "https://api.mistral.ai/v1", "default_model": "mistral-small-latest"},
    "deepseek": {"label": "DeepSeek", "kind": "openai", "env": ["DEEPSEEK_API_KEY"],
                 "base": "https://api.deepseek.com/v1", "default_model": "deepseek-chat"},
    "fireworks": {"label": "Fireworks", "kind": "openai", "env": ["FIREWORKS_API_KEY"],
                  "base": "https://api.fireworks.ai/inference/v1",
                  "default_model": "accounts/fireworks/models/llama-v3p3-70b-instruct"},
    "perplexity": {"label": "Perplexity", "kind": "openai", "env": ["PERPLEXITY_API_KEY"],
                   "base": "https://api.perplexity.ai", "default_model": "sonar"},
    "ollama": {"label": "Ollama (local)", "kind": "ollama", "env": [],
...
def _call_openai_compatible(cfg, model, system, prompt, max_tokens) -> Dict[str, Any]:
    ...
    base = os.getenv("AGENTLENS_OPENAI_BASE_URL", cfg["base"]).rstrip("/")
    ...
    try:
        ...
        raise ProviderError(f"{cfg['label']} returned {e.response.status_code}: {_err_body(e.response)}") from e
    ...
    return {"text": (choice.get("message") or {}).get("content", "") or "",
            ...
            "model": data.get("model", model), "provider": cfg["id"]}
...
def _call_anthropic(cfg, model, system, prompt, max_tokens) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ProviderError(f"Anthropic returned {e.response.status_code}: {_err_body(e.response)}") from e
    except Exception as e:
        raise ProviderError(f"Anthropic call failed: {e}") from e
    data = r.json()
    text = "".join(b.get("text", "") for b in data.get("content", []) if b.get("type") == "text")
    usage = data.get("usage") or {}
    return {"text": text, "input_tokens": usage.get("input_tokens"),
...
def available_providers() -> List[Dict[str, Any]]:
    ...
    for pid, cfg in PROVIDERS.items():
        ...
        out.append({"id": pid, "label": cfg["label"], "ready": ready,
